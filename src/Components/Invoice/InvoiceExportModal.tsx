...
export default function InvoiceExportModal({
  ...
  const { exports, loading, createExport, refreshExports, pollExportStatus } =
    useInvoiceExport();
  ...
  const handleCreateExport = async () => {
    ...
    if (selectedStatuses.length > 0)
      params.invoiceStatus = selectedStatuses as InvoiceStatus[];
    if (selectedTypes.length > 0)
      params.invoiceTypes = selectedTypes as InvoiceType[];
        ...
        <Paper p="md" withBorder>
          ...
          <Stack gap="md">
            ...
            <Select
              ...
              data={InvoiceStatusOptions.map((status) => ({
                value: status.value,
                label: status.label,
              }))}
